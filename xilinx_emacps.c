 ...
 *
 * Author: Xilinx, Inc.
 * Porter: Wilken Gottwalt (beyond kernel 4.14)
 *
 ...
 *
 * TODO:
 * 1. JUMBO frame is not enabled per EPs spec. Please update it if this support is added in and set
 *    MAX_MTU to 9000.
 * 2. PTP slave mode: Currently master and slave sync is tested for 111MHz and 125 MHz ptp clock.
 *    Need to test with various other clock frequencies.
 * 3. PTP in general: this is experimental and will never work flawlessly. The hardware is broken
 *    and does not really fully support PTP because of a wrong design of the timestamping registers
 *    on the silicon level. If you can live with some messed up stamps and filter them out on the
 *    software level (by some nice logic), this is actually usable.
 */
...
#define to_net_local(_nb) container_of(_nb, struct net_local, clk_rate_change_nb)
...
static int xemacps_mdio_read(struct mii_bus *bus, int mii_id, int phyreg)
{
	...
	value = xemacps_read(lp->baseaddr, XEMACPS_PHYMNTNC_OFFSET) & XEMACPS_PHYMNTNC_DATA_MASK;
 ...
 * 0x00020000.
 */
static int xemacps_mdio_write(struct mii_bus *bus, int mii_id, int phyreg, u16 value)
{
	struct net_local *lp = bus->priv;
	u32 regval;
...
static void xemacps_adjust_link(struct net_device *ndev)
{
	...
	if (phydev->link) {
		if (lp->speed != phydev->speed || lp->duplex != phydev->duplex) {
			regval = xemacps_read(lp->baseaddr, XEMACPS_NWCFG_OFFSET);
			regval &= ~(XEMACPS_NWCFG_FDEN_MASK | XEMACPS_NWCFG_1000_MASK |
			...
			if (phydev->speed == SPEED_1000) {
				...
				xemacps_set_freq(lp->devclk, 125000000, &lp->pdev->dev);
			} else if (phydev->speed == SPEED_100) {
				regval |= XEMACPS_NWCFG_100_MASK;
				gmii2rgmii_reg |= XEMACPS_GMII2RGMII_SPEED100;
				xemacps_set_freq(lp->devclk, 25000000, &lp->pdev->dev);
			} else if (phydev->speed == SPEED_10) {
				xemacps_set_freq(lp->devclk, 2500000, &lp->pdev->dev);
			} else {
				dev_err(&lp->pdev->dev, "%s: unknown PHY speed %d\n", __func__,
					phydev->speed);
				return;
			}
			...
			if (gmii2rgmii_phydev) {
				xemacps_mdio_write(lp->mii_bus, gmii2rgmii_phydev->mdio.addr,
						   XEMACPS_GMII2RGMII_REG_NUM, gmii2rgmii_reg);
			}
...
static int xemacps_mii_probe(struct net_device *ndev)
{
	...
	if (lp->gmii2rgmii_phy_node) {
		ret = phy_attach_direct(lp->ndev, phydev, 0, 0);
		if (unlikely(ret)) {
	...
	phydev = of_phy_connect(lp->ndev, lp->phy_node, &xemacps_adjust_link, 0, lp->phy_interface);
	...
	dev_dbg(&lp->pdev->dev, "GEM: phydev %p, phydev->phy_id 0x%x, phydev->addr 0x%x\n",
		phydev, phydev->phy_id, phydev->mdio.addr);

	ethtool_convert_legacy_u32_to_link_mode(phydev->supported, SUPPORTED_Autoneg |
						SUPPORTED_TP | SUPPORTED_MII |
						SUPPORTED_Pause | SUPPORTED_Asym_Pause |
						SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full |
						SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full |
						SUPPORTED_1000baseT_Full);
	ethtool_convert_legacy_u32_to_link_mode(phydev->advertising, SUPPORTED_Autoneg |
						SUPPORTED_TP | SUPPORTED_MII |
						SUPPORTED_Pause | SUPPORTED_Asym_Pause |
						SUPPORTED_10baseT_Half | SUPPORTED_10baseT_Full |
						SUPPORTED_100baseT_Half | SUPPORTED_100baseT_Full |
...
static void xemacps_update_hwaddr(struct net_local *lp)
{
	...
	if (is_valid_ether_addr(addr)) {
		dev_addr_set(lp->ndev, addr);
	} else {
		dev_info(&lp->pdev->dev, "invalid address, use random\n");
		eth_hw_addr_random(lp->ndev);
		dev_info(&lp->pdev->dev, "MAC updated %02x:%02x:%02x:%02x:%02x:%02x\n",
			 lp->ndev->dev_addr[0], lp->ndev->dev_addr[1], lp->ndev->dev_addr[2],
			 lp->ndev->dev_addr[3], lp->ndev->dev_addr[4], lp->ndev->dev_addr[5]);
	}
}
...
static void xemacps_set_hwaddr(struct net_local *lp)
{
	...
	regvall = xemacps_read(lp->baseaddr, XEMACPS_LADDR1L_OFFSET);
	regvalh = xemacps_read(lp->baseaddr, XEMACPS_LADDR1H_OFFSET);
	dev_dbg(&lp->pdev->dev, "MAC 0x%08x, 0x%08x, %02x:%02x:%02x:%02x:%02x:%02x\n", regvall,
		regvalh, regvall & 0xff, (regvall >> 8) & 0xff, (regvall >> 16) & 0xff,
		regvall >> 24, regvalh & 0xff, regvalh >> 8);
#endif
}
...
static void xemacps_reset_hw(struct net_local *lp)
{
	 ...
	 */
	xemacps_write(lp->baseaddr, XEMACPS_NWCTRL_OFFSET, XEMACPS_NWCTRL_STATCLR_MASK |
 ...
 * Return: None
 */
static inline void xemacps_ptp_read(struct net_local *lp, struct timespec64 *ts)
{
 ...
 * Return: None
 */
static inline void xemacps_ptp_write(struct net_local *lp, const struct timespec64 *ts)
{
...
static void xemacps_systim_to_hwtstamp(struct net_local *lp,
				       struct skb_shared_hwtstamps *shhwtstamps, u64 regval)
{
	u64 ns;
 ...
 * Return: None
 */
static void xemacps_rx_hwtstamp(struct net_local *lp, struct sk_buff *skb, u32 msg_type)
{
	u32 sec, nsec;
	u64 time64;
 ...
 * Return: None
 */
static void xemacps_tx_hwtstamp(struct net_local *lp, struct sk_buff *skb, u32 msg_type)
{
	u32 sec, nsec;
	u64 time64;
 ...
 */
static int xemacps_ptp_enable(struct ptp_clock_info *ptp, struct ptp_clock_request *rq, int on)
{
	return -EOPNOTSUPP;
}
 ...
 * Return: Always returns zero
 */
static int xemacps_ptp_settime(struct ptp_clock_info *ptp, const struct timespec64 *ts)
{
	unsigned long flags;
	struct net_local *lp = container_of(ptp, struct net_local, ptp_caps);
	u64 ns;
...
static int xemacps_ptp_adjfine(struct ptp_clock_info *ptp, long scaled_ppm)
{
	const long ppb = scaled_ppm_to_ppb(scaled_ppm);

	return xemacps_ptp_adjfreq(ptp, ppb);
}
...
static void xemacps_ptp_init(struct net_local *lp)
{
	...
	lp->ptp_caps.pps = 0;
	lp->ptp_caps.adjfine = xemacps_ptp_adjfine;
	...
	timer_setup(&lp->time_keep, xemacps_time_keep, 0);
	mod_timer(&lp->time_keep, jiffies + HZ);
	...
	xemacps_write(lp->baseaddr, XEMACPS_1588INC_OFFSET, lp->tmr_add);
	ktime_get_real_ts64(&now);
	xemacps_ptp_write(lp, (const struct timespec64 *)&now);
	timecounter_init(&lp->tc, &lp->cc, ktime_to_ns(ktime_get_real()));
...
static int xemacps_rx(struct net_local *lp, int budget)
{
	...
	while (numbdfree < budget) {
		...
		new_skb_baddr = (u32)dma_map_single(lp->ndev->dev.parent, new_skb->data,
		...
		dma_unmap_single(lp->ndev->dev.parent, lp->rx_skb[lp->rx_bd_ci].mapping,
		...
		if (lp->hwtstamp_config.rx_filter == HWTSTAMP_FILTER_ALL &&
			 ...
			 * descriptor to identify them. This is entirely the wrong place to be
			 * parsing UDP headers, but some minimal effort must be made.
			 * NOTE: the below parsing of ip_proto and dest_port depend on the use of
			 * Ethernet_II encapsulation, IPv4 without any options.
			 */
			skb_copy_from_linear_data_offset(skb, XEMACPS_IP_PROTO_OFFSET,
							 &transport_poto, 1);
			if (transport_poto == IPPROTO_UDP) {
				skb_copy_from_linear_data_offset(skb, XEMACPS_UDP_PORT_OFFSET,
								 &dst_port, 2);
				if (ntohs(dst_port) == XEMACPS_PTP_EVENT_PORT_NUM) {
...
static int xemacps_rx_poll(struct napi_struct *napi, int budget)
{
	...
	while (1) {
		...
		if (lp->rx_bd[lp->rx_bd_ci].addr & XEMACPS_RXBUF_NEW_MASK) {
			napi_schedule(napi);
			xemacps_write(lp->baseaddr, XEMACPS_IDR_OFFSET, XEMACPS_IXR_FRAMERX_MASK);
			continue;
		}
		break;
	}
...
static void xemacps_tx_poll(unsigned long data)
{
	...
	while (numbdsinhw) {
		if ((cur_p->ctrl & XEMACPS_TXBUF_USED_MASK) != XEMACPS_TXBUF_USED_MASK) {
			if (!isfrag)
				break;
		}
		...
		if (skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP) {
			...
			skb_copy_from_linear_data_offset(skb, XEMACPS_TX_PTPHDR_OFFSET, &msg_type,
							 1);
			xemacps_tx_hwtstamp(lp, skb, msg_type & 0x2);
		}
#endif /* CONFIG_XILINX_PS_EMAC_HWTSTAMP */

		dma_unmap_single(&lp->pdev->dev, rp->mapping, rp->len, DMA_TO_DEVICE);
		rp->skb = NULL;
		dev_kfree_skb(skb);
...
static irqreturn_t xemacps_interrupt(int irq, void *dev_id)
{
	...
	while (regisr) {
		if (regisr & (XEMACPS_IXR_TXCOMPL_MASK | XEMACPS_IXR_TX_ERR_MASK))
...
static void xemacps_clean_rings(struct net_local *lp)
{
	...
	for (i = 0; i < XEMACPS_RECV_BD_CNT; i++) {
		if (lp->rx_skb && lp->rx_skb[i].skb) {
			dma_unmap_single(lp->ndev->dev.parent, lp->rx_skb[i].mapping,
					 lp->rx_skb[i].len, DMA_FROM_DEVICE);
	...
	for (i = 0; i < XEMACPS_SEND_BD_CNT; i++) {
		if (lp->tx_skb && lp->tx_skb[i].skb) {
			dma_unmap_single(lp->ndev->dev.parent, lp->tx_skb[i].mapping,
...
static void xemacps_descriptor_free(struct net_local *lp)
{
	...
	if (lp->rx_bd) {
		dma_free_coherent(&lp->pdev->dev, size, lp->rx_bd, lp->rx_bd_dma);
		lp->rx_bd = NULL;
	}

	size = XEMACPS_SEND_BD_CNT * sizeof(struct xemacps_bd);
	if (lp->tx_bd) {
		dma_free_coherent(&lp->pdev->dev, size, lp->tx_bd, lp->tx_bd_dma);
		lp->tx_bd = NULL;
	}
}
...
static int xemacps_descriptor_init(struct net_local *lp)
{
	...
	size = XEMACPS_RECV_BD_CNT * sizeof(struct xemacps_bd);
	lp->rx_bd = dma_alloc_coherent(&lp->pdev->dev, size, &lp->rx_bd_dma, GFP_KERNEL);
	if (!lp->rx_bd)
		goto err_out;
	dev_dbg(&lp->pdev->dev, "RX ring %d bytes at 0x%llx mapped %p\n", size,
		(long long)lp->rx_bd_dma, lp->rx_bd);
	...
	size = XEMACPS_SEND_BD_CNT * sizeof(struct xemacps_bd);
	lp->tx_bd = dma_alloc_coherent(&lp->pdev->dev, size, &lp->tx_bd_dma, GFP_KERNEL);
	if (!lp->tx_bd)
		goto err_out;
	dev_dbg(&lp->pdev->dev, "TX ring %d bytes at 0x%llx mapped %p\n", size,
		(long long)lp->tx_bd_dma, lp->tx_bd);
 ...
 */
static void xemacps_resetrx_for_no_rxdata(struct net_local *lp)
{
...
static int xemacps_open(struct net_device *ndev)
{
	...
	if (rc) {
		dev_err(&lp->pdev->dev, "Unable to allocate DMA memory, rc %d\n", rc);
		return rc;
	}

	rc = pm_runtime_get_sync(&lp->pdev->dev);
	if (rc < 0) {
		dev_err(&lp->pdev->dev, "pm_runtime_get_sync() failed, rc %d\n", rc);
		goto err_free_rings;
	}
	...
	timer_setup(&lp->gen_purpose_timer, xemacps_gen_purpose_timerhandler, 0);
	lp->timerready = true;
...
{
	struct net_local *lp = container_of(data, struct net_local, txtimeout_reinit);
	int rc;
 ...
 * @ndev: network interface device structure
 */
static void xemacps_tx_timeout(struct net_device *ndev, unsigned int txqueue)
{
	struct net_local *lp = netdev_priv(ndev);

	dev_err(&lp->pdev->dev, "transmit timeout %lu ms, resetting...\n",
...
static int xemacps_start_xmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	for (i = 0; i < nr_frags; i++) {
		if (i == 0) {
			len = skb_headlen(skb);
			mapping = dma_map_single(&lp->pdev->dev, skb->data, len, DMA_TO_DEVICE);
		} else {
			len = skb_frag_size(frag);
			virt_addr = skb_frag_address(frag);
			mapping = dma_map_single(&lp->pdev->dev, virt_addr, len, DMA_TO_DEVICE);
			frag++;
			skb_get(skb);
		}
	...
	spin_lock_irqsave(&lp->nwctrlreg_lock, flags);
	regval = xemacps_read(lp->baseaddr, XEMACPS_NWCTRL_OFFSET);
	xemacps_write(lp->baseaddr, XEMACPS_NWCTRL_OFFSET, regval | XEMACPS_NWCTRL_STARTTX_MASK);
	spin_unlock_irqrestore(&lp->nwctrlreg_lock, flags);
...
static u32 get_bit(u8 *mac, u8 bit)
{
	u8 byte;
...
static u32 calc_mac_hash(u8 *mac)
{
	...
	for (index_bit = 5; index_bit >= 0; index_bit--) {
		hash_index |= (get_bit(mac,  mac_bit) ^ get_bit(mac, mac_bit + 6) ^
			      get_bit(mac, mac_bit + 12) ^ get_bit(mac, mac_bit + 18) ^
			      get_bit(mac, mac_bit + 24) ^ get_bit(mac, mac_bit + 30) ^
			      get_bit(mac, mac_bit + 36) ^ get_bit(mac, mac_bit + 42)) << index_bit;
		mac_bit--;
	}
...
static void xemacps_set_hashtable(struct net_device *ndev)
{
	...
	netdev_for_each_mc_addr(curr, ndev) {
		...
		if (hash_index >= XEMACPS_MAX_HASH_BITS) {
			dev_err(&lp->pdev->dev, "hash calculation out of range %d\n", hash_index);
			break;
		}
		if (hash_index < 32)
 ...
 */
static void xemacps_get_drvinfo(struct net_device *ndev, struct ethtool_drvinfo *ed)
{
	struct net_local *lp = netdev_priv(ndev);
 ...
 */
static void xemacps_get_ringparam(struct net_device *ndev, struct ethtool_ringparam *ering,
				  struct kernel_ethtool_ringparam *kernel_ering,
				  struct netlink_ext_ack *extack)
{
	memset(ering, 0, sizeof(struct ethtool_ringparam));

	ering->rx_max_pending = XEMACPS_RECV_BD_CNT;
	ering->tx_max_pending = XEMACPS_SEND_BD_CNT;
	ering->rx_pending = 0;
	ering->tx_pending = 0;
}
 ...
 * @ewol: wol status
 */
static void xemacps_get_wol(struct net_device *ndev, struct ethtool_wolinfo *ewol)
{
	struct net_local *lp = netdev_priv(ndev);
	u32 regval;
 ...
 */
static int xemacps_set_wol(struct net_device *ndev, struct ethtool_wolinfo *ewol)
{
	struct net_local *lp = netdev_priv(ndev);
	u32 regval;
	...
	regval  = xemacps_read(lp->baseaddr, XEMACPS_WOL_OFFSET);
	regval &= ~(XEMACPS_WOL_MCAST_MASK | XEMACPS_WOL_ARP_MASK | XEMACPS_WOL_SPEREG1_MASK |
 ...
 */
static void xemacps_get_pauseparam(struct net_device *ndev, struct ethtool_pauseparam *epauseparm)
{
	struct net_local *lp = netdev_priv(ndev);
	u32 regval;
 ...
 * note: hardware supports only tx flow control
 */
static int xemacps_set_pauseparam(struct net_device *ndev, struct ethtool_pauseparam *epauseparm)
{
	struct net_local *lp = netdev_priv(ndev);
	u32 regval;

	if (netif_running(ndev)) {
		dev_err(&lp->pdev->dev, "Please stop netif before apply configruation\n");
		return -EFAULT;
	}
...
static int xemacps_get_ts_info(struct net_device *dev, struct kernel_ethtool_ts_info *info)
#else
static int xemacps_get_ts_info(struct net_device *dev, struct ethtool_ts_info *info)
#endif
{
...
static const struct ethtool_ops xemacps_ethtool_ops = {
...
#ifdef CONFIG_XILINX_PS_EMAC_HWTSTAMP
static int xemacps_hwtstamp_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
...
static int xemacps_ioctl(struct net_device *ndev, struct ifreq *rq, int cmd)
{
	...
	struct phy_device *phydev = lp->phy_dev;
#ifdef CONFIG_XILINX_PS_EMAC_HWTSTAMP
	struct hwtstamp_config *cfg = &lp->hwtstamp_config;
#endif /* CONFIG_XILINX_PS_EMAC_HWTSTAMP */
	...
	case SIOCGHWTSTAMP:
		return copy_to_user(rq->ifr_data, cfg, sizeof(*cfg)) ? -EFAULT : 0;
#endif /* CONFIG_XILINX_PS_EMAC_HWTSTAMP */
	default:
...
static int xemacps_probe(struct platform_device *pdev)
{
	...
	netif_napi_add_weight(ndev, &lp->napi, xemacps_rx_poll, XEMACPS_NAPI_WEIGHT);
	...
	lp->aperclk = devm_clk_get(&pdev->dev, "pclk");
	if (IS_ERR(lp->aperclk)) {
		dev_err(&pdev->dev, "pclk clock not found.\n");
		rc = PTR_ERR(lp->aperclk);
		goto err_out_unregister_netdev;
	}
	lp->devclk = devm_clk_get(&pdev->dev, "tx_clk");
	if (IS_ERR(lp->devclk)) {
		dev_err(&pdev->dev, "tx_clk clock not found.\n");
	...
	if (rc) {
		dev_err(&pdev->dev, "Unable to enable TX_CLK clock.\n");
	...
	lp->gmii2rgmii_phy_node = of_parse_phandle(lp->pdev->dev.of_node, "gmii2rgmii-phy-handle",
						   0);
	rc = of_get_phy_mode(lp->pdev->dev.of_node, &lp->phy_interface);
	if (rc < 0) {
	...
	rc = of_get_mac_address(lp->pdev->dev.of_node, mac_address);
	if (!rc) {
		dev_addr_set(lp->ndev, mac_address);
		xemacps_set_hwaddr(lp);
	} else {
		xemacps_update_hwaddr(lp);
	}

	tasklet_init(&lp->tx_bdreclaim_tasklet, xemacps_tx_poll, (unsigned long)ndev);
	tasklet_disable(&lp->tx_bdreclaim_tasklet);
	...
	rc = devm_request_irq(&pdev->dev, ndev->irq, &xemacps_interrupt, 0, ndev->name, ndev);
	if (rc) {
 ...
 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,11,0)
static void xemacps_remove(struct platform_device *pdev)
#else
...
{
	struct platform_device *pdev = container_of(device, struct platform_device, dev);
	struct net_device *ndev = platform_get_drvdata(pdev);
	struct net_local *lp = netdev_priv(ndev);
...
static const struct dev_pm_ops xemacps_dev_pm_ops = {
	...
	SET_RUNTIME_PM_OPS(xemacps_runtime_suspend, xemacps_runtime_resume, xemacps_runtime_idle)
};
...
static const struct net_device_ops netdev_ops = {
	...
	.ndo_eth_ioctl		= xemacps_ioctl,
	.ndo_tx_timeout		= xemacps_tx_timeout,
	.ndo_get_stats		= xemacps_get_stats,
};

static const struct of_device_id xemacps_of_match[] = {
